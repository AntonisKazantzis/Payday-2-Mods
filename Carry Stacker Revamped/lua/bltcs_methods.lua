...
function BLT_CarryStacker:ResetSettings()
    ...
    self.settings.toggle_penalties = false
...
--[[
    ...
    movement_penalties or the host movement_penalties according to the
    game state.
...
function BLT_CarryStacker:getWeightForType(carry_id, logger)
    ...
    logger("Request to get the weight of carry " .. tostring(carry_id))

    if not self:IsPenaltiesEnabled() then
        logger("Penalties are disabled. Returning weight of 1")
        return 1
    end
...
function BLT_CarryStacker:HostStealthOnly()
    local logger = BLT_CarryStacker.Log
...
function BLT_CarryStacker:IsPenaltiesEnabled()
    local logger = BLT_CarryStacker.Log
    logger("Request to return settings.toggle_penalties. Its value " ..
        "is " .. tostring(self.settings.toggle_penalties))
    return self.settings.toggle_penalties
end
...
function BLT_CarryStacker:CanCarry(carry_id, logger)
    logger = logger or BLT_CarryStacker.Log
    logger("Request to check whether the player can carry " .. tostring(carry_id))
    ...
    if not self:IsPenaltiesEnabled() then
        logger("The mod has penalties disabled. Can carry more bags")
        return true
    end
...
--[[
    Update the HUD's carry symbol, indicating the amount of bags
...
--[[
    Toggle the disable penalties setting.
]]
function BLT_CarryStacker:togglePenalties(state)
    local logger = BLT_CarryStacker.Log
    logger("Toggling disable penalties to " .. tostring(state))
    self:SetSetting("toggle_penalties", state)
end
