...
function PlayerCarry:_perform_jump(jump_vec)
	...
	if BLT_CarryStacker:IsPenaltiesEnabled() and not managers.player:has_category_upgrade("carry", "movement_penalty_nullifier") then
...
function PlayerCarry:_get_max_walk_speed(...)
	...
	if not BLT_CarryStacker:IsPenaltiesEnabled() or managers.player:has_category_upgrade("carry", "movement_penalty_nullifier") then
...
function PlayerCarry:_get_walk_headbob(...)
	...
	local weight = BLT_CarryStacker:IsPenaltiesEnabled() and BLT_CarryStacker.weight or 1
	logger("Returning super walk headbob penalised by the " ..
		"current weight: " .. tostring(weight))
	local result = PlayerCarry.super._get_walk_headbob(self, ...) * weight
...
function PlayerCarry:_check_action_run(...)
	...
	if (not BLT_CarryStacker:IsPenaltiesEnabled() or BLT_CarryStacker.weight < 0.75) 
			or managers.player:has_category_upgrade("carry", "movement_penalty_nullifier") then
		logger("The player's movement is not penalised or " ..
			"the current weight is less than 0.75. Calling super check_run")
